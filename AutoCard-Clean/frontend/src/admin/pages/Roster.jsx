...
const Roster = () => {
 ...
 const prevMonth = () => {
  ...
  // Use LOCAL date here because form dates are calendar dates,
  // not UTC timestamps.
  const dateStr = [
  ...
  const deleteAllRoster = async () => {
    ...
    try {
      ...
      toast.error(err.message || "Failed to delete all roster entries.");
      await loadRoster();
    } finally {
    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Plan and assign employee work schedules.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={
              entries.length
                ? `Delete all ${entries.length} loaded roster entries`
                : "No roster entries to delete"
            }
          >
            {deletingAll ? (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={new Set(entries.map((e) => e.employeeId)).size}
          tone="blue"
        />
        <StatCard
          icon={Clock}
          label="Shifts Used"
          value={new Set(entries.map((e) => e.shiftId)).size}
          tone="green"
        />
        <StatCard
          icon={MapPin}
      ...
      <div className="flex items-center justify-between rounded-2xl bg-background border border-border card-shadow px-5 py-3">
        ...
        <div className="text-center">
          ...
          <div className="text-xs text-muted-foreground">
            {entries.length} roster {entries.length === 1 ? "entry" : "entries"}
          </div>
        </div>
        <button
          onClick={nextMonth}
            ...
            >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((p) => ({
                      ...
                      toDate:
                        p.toDate && e.target.value > p.toDate
                          ? e.target.value
                          : p.toDate,
                    }))
                  }
              ...
              <div>
                ...
                <select
                  ...
                  value={form.shiftId}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, shiftId: e.target.value }))
                  }
                  required
                >
        ...
        <div className="rounded-2xl bg-background border border-border card-shadow overflow-hidden">
          ...
          <div className="grid grid-cols-7">
            ...
            {Array.from({ length: firstDayOfWk }).map((_, i) => (
              <div
                key={`e${i}`}
                className="min-h-[100px] border-r border-b border-border/40 bg-secondary/10"
              />
            ))}
            {/* Day cells */}
                    ...
                    >
                      <td className="px-5 py-3 font-semibold">
                        ...
                        {e.employee?.employeeCode && (
                          <span className="ml-1.5 text-xs text-primary/70 font-mono">
                            #{e.employee.employeeCode}
                          </span>
                        )}
                      </td>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Remove{" "}
                  <strong>{deleteTarget.employee?.user?.fullName}</strong>'s
                  entry for <strong>{fmtDate(deleteTarget.date)}</strong>? This
                  cannot be undone.
                </p>
              </div>
            </div>
