      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            This cannot be undone. Employees assigned to this record will lose
            the assignment.
          </p>
        </div>
      </div>
  ...
  const load = useCallback(async () => {
    ...
    try {
      const d = await apiGet(
        `/shifts?limit=100&search=${encodeURIComponent(search)}`,
      );
      setShifts(d.data ?? []);
    } catch (err) {
                ...
                {shifts.map((s) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-center">
                      <span
                        className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${s.isActive ? "bg-green-100 text-green-700" : "bg-rose-100 text-rose-600"}`}
                      >
...
const LocationsPanel = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const d = await apiGet(
        `/locations?limit=100&search=${encodeURIComponent(search)}`,
      );
      setLocations(d.data ?? []);
    } catch (err) {
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Manage office and work site locations.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
              ...
              <tbody className="divide-y divide-border">
                {locations.map((l) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-center">
                      <span
                        className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${l.isActive ? "bg-green-100 text-green-700" : "bg-rose-100 text-rose-600"}`}
                      >
